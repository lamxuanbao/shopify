    ...
    <div class="container-fluid h-100">
        <div class="row justify-content-center h-100">
            <div class="col-md-4 col-xl-3 chat">
                <div class="card mb-sm-3 mb-md-0 contacts_card">
                    <div class="card-body contacts_body">
                        <ui class="contacts message-list">
                            @foreach($page->conversations as $conversation)
                                <li onclick="loadMessage('{{$conversation->id}}');">
                                    <div class="d-flex bd-highlight">
                                        ...
                                        <div class="user_info">
                                            @foreach($conversation->users as $user)
                                                @if($user->provider_id != $page->provider_id)
                                                    <span>{{$user->name}}</span>
            ...
            <div class="col-md-8 col-xl-6 chat">
                <div class="card" id="message_inbox">
                </div>
            </div>
        </div>
        <br>
...
@push('scripts')
    <script type="text/javascript">
        ...
        var pageId = window.page.provider_id;
        var conversationId = null;
        $(document).ready(function () {
            var database = firebase.database();
            database.ref('social_page_conversation_details').on('child_added', function (childSnapshot) {
                var data = childSnapshot.val();
                var html = '';
                if (data.from.id != window.page.provider_id) {
                    html = '<div class="d-flex justify-content-start mb-4">\n' +
                        ...
                        '                </div>\n';
                    if (data.message != '') {
                        html += '<div class="msg_cotainer">' + data.message + '</div>';
                    } else if (data.sticker != '') {
                        html += '<img src="' + data.sticker + '" width="100" height="100">';
                    } else {
                        var attachments = data.attachments;
                        attachments.forEach(function (attachment) {
                            if (attachment['image_data']) {
                                html += '<img src="' + attachment['image_data']['preview_url'] + '" width="100" height="100">';
                            }
                            if (attachment['video_data']) {
                                html += '<video width="320" height="240" controls>\n' +
                                    '                    <source src="' + attachment['video_data']['url'] + '" type="video/mp4">\n' +
                                    '                        Your browser does not support the video tag.\n' +
                                    '                    </video>';
                            }
                            if (attachment['file_url']) {
                                html += '<a href="' + attachment['file_url'] + '">\n' +
                                    attachment['name'] +
                                    '                        </a>';
                            }
                        });
                ...
                $("body #message_body").append(html);
                database.ref('social_page_conversation_details/' + data.id).remove();
            });
        });
        $(document).on('click', '#send_btn', function (event) {
            const message_reply = $("#message_reply").val();
            ...
            $.ajax({
                type: 'POST',
                url: '/message/' + pageId + '/' + conversationId,
                data: {
        ...
        function loadMessage(loadConversationId) {
            conversationId = loadConversationId;
            console.log(loadConversationId);
            $.ajax({
                type: 'GET',
                url: '/message/' + pageId + '/' + conversationId,
                success: function (result) {
                    $("#message_inbox").html(result.view);
                }
            })
        }
    </script>
